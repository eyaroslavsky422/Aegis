
export const PATIENTS = [
  {
    id: "AMB-2401",
    name: "John Martinez",
    age: 67,
    condition: "Sepsis",
    ambulancePos: [37.7849, -122.4194], // SF
    hospitalPos: [37.7886, -122.4098],
    eta: 8,
    criticalParams: ["lactate", "map"],
  },
  {
    id: "AMB-2402",
    name: "Sarah Chen",
    age: 52,
    condition: "Sepsis",
    ambulancePos: [37.7749, -122.4294],
    hospitalPos: [37.7886, -122.4098],
    eta: 12,
    criticalParams: ["qsofa"],
  },
  {
    id: "AMB-2403",
    name: "Robert Williams",
    age: 71,
    condition: "Pneumonia",
    ambulancePos: [37.7949, -122.4394],
    hospitalPos: [37.7886, -122.4098],
    eta: 6,
    criticalParams: ["hr", "rr"],
  },
  {
    id: "AMB-2404",
    name: "Maria Rodriguez",
    age: 45,
    condition: "Asthma Attack",
    ambulancePos: [37.7649, -122.4094],
    hospitalPos: [37.7886, -122.4098],
    eta: 15,
    criticalParams: [],
  },
];

export const AWAITING_PICKUP = [
  {
    id: "PEND-301",
    name: "David Thompson",
    age: 58,
    condition: "Sepsis",
    location: [37.7699, -122.4469], // Patient location
    hospitalPos: [37.7886, -122.4098],
    estimatedArrival: 5, // minutes until ambulance arrives
    criticalParams: ["lactate", "temp"],
  },
];
